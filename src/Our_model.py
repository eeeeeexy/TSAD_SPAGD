...
def dot_product_decode(Z):
    ...
    std = Z_.std()
    Z_ = (Z_ - mean) / (std + 1e-8)
    # threshold_1 = torch.quantile(z, 0.05)
    Z_1 = Z_.detach()
    A_pred = torch.sigmoid(Z_1) # 可以换成tahn
    return A_pred
...
# class graph_constructor_init(nn.Module):
#     '''
...
#         super(graph_constructor_init, self).__init__()
#         self.nnodes = nnodes
...
#         kkk = torch.mm(nodevec, nodevec.transpose(1,0))
        
#         adj = F.relu(torch.tanh(self.alpha*kkk))
#         mask = torch.zeros(idx.size(0), idx.size(0)).to(device)
#         mask.fill_(float('0'))
#         # mask = torch.zeros_like(nodevec1, device=device)
...
class graph_constructor(nn.Module):
    ...
    def forward(self, inputs, inputs_init, outputs_init, idx):
        ...
        nodevec = nodevec.transpose(1,0)
        nodevec = nodevec.reshape(inputs.shape[-2], nodevec.size(1)*nodevec.size(2))
        # kk = torch.mm(nodevec, nodevec.transpose(1,0))

        kkk = dot_product_decode(nodevec)

        # reconstruction gap and sigmiod it to rescale
...
# class stgnn_gat_base(nn.Module):
...
#         super(stgnn_gat_base, self).__init__()
...
#         self.gc = graph_constructor_init(num_nodes, subgraph_size, node_dim, devices, alpha=tanhalpha, static_feat=static_feat).to(device)
...
#             if self.buildA_true:
#                 if idx is None:
#                     adp = self.gc(input, self.idx).to(device)
#                 else:
#                     adp = self.gc(input, idx).to(device)
#             else:
...
#         # print(f'adp: {adp}; non-zero num: {torch.count_nonzero(adp).item()}')
...
#                 for t in range(T):
#                     x_t = x[:, :, :, t]          # [B, C, N]
#                     x_t = x_t.permute(0, 2, 1)   # [B, N, C]
#                     x_t = x_t.reshape(B * N, C)  # flatten for GAT: [B*N, C]
...
#                     edge_index_all = torch.cat(edge_index_batch, dim=1)  # [2, B * num_edges]

#                     # Apply GAT
#                     x_t_out = self.gconv1[i](x_t, edge_index_all) + self.gconv2[i](x_t, edge_index_all)
#                     # import pdb; pdb.set_trace()
#                     x_t_out = x_t_out.view(B, N, -1).permute(0, 2, 1)  # [B, C_out, N]
#                     x_out.append(x_t_out.unsqueeze(-1))  # [B, C_out, N, 1]
#                 # Stack over time
#                 x = torch.cat(x_out, dim=-1)  # [B, C_out, N, T]
...
#             # x = x.view(16, 1, 51, -1).permute(0, 3, 2, 1)
...
class SPAGD_model(nn.Module):
    def __init__(self, dataset, device, args):
        ...
        seq_in_len_t = int(args.window_size/self.chunk_num)  # 25
        seq_out_len = 20  
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        inputs_s = torch.unsqueeze(inputs_s, 1) # B, 1, node, window
        ...
        # w/o spatial gragh
        # inputs_x, inputs_adp = self.stgnn_s_input(inputs_s)
        # outputs_x, outputs_adp = self.stgnn_s_input(outputs_s)
        ...
        for idx_, chunk_ti in enumerate(input_chunks_ti_):
            ...
            # w/o temporal gragh
            # input_ti, inputs_adp_ti = self.stgnn_t_input(chunk_ti)
        ...
        # final_input_predict = torch.sigmoid(final_input_predict)
        # print(f'final_input_predict: {final_input_predict}')
        # import pdb; pdb.set_trace()
        # final_input_predict = final_input_predict.unsqueeze(1)

        # input_st_predict = torch.where(final_input_predict >= threshold, torch.tensor(1.0), torch.tensor(0.0))
        ...
        for idx_, chunk_ti in enumerate(output_chunks_ti_):
            ...
            # w/o temporal gragh
            # output_ti, outputs_adp_ti = self.stgnn_t_input(chunk_ti)
        ...
        # final_output_predict = torch.sigmoid(final_output_predict)

        # output_st_predict = torch.where(final_output_predict >= threshold, torch.tensor(1.0), torch.tensor(0.0))
